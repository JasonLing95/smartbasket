...
def extract_receipt_data_via_llm(text_blob: str) -> dict | None:
    ...
    system_instruction = (
        ...
        "3. CRITICAL - LOYALTY DISCOUNTS, MEAL DEALS & POOLED DISCOUNTS: Discounts apply to the parent item ABOVE them. Capture the positive base price in 'base_price', and the absolute value of the discount in 'discount_applied' (e.g., 0.50). Determine the name of the discount program (e.g., 'Nectar Price Saving') and output it as 'discount_type'.\n"
        "   - POOLED MULTIPLIERS: If a discount line contains a multiplier (e.g., '2 X Nectar Price Saving -£2.80'), DO NOT treat this as a product quantity. Apply the FULL absolute discount value (2.80) to the SINGLE item immediately above it. Never attempt to divide the discount across multiple items, and NEVER drop the surrounding items to balance the math.\n"
