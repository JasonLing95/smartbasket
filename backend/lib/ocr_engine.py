...
def extract_receipt_data_via_llm(text_blob: str) -> dict | None:
    ...
    system_instruction = (
        ...
        "5. CRITICAL - DATE EXTRACTION: Extract the exact printed transaction date. You MUST assume all dates are printed in standard UK format (DD/MM/YY). You MUST explicitly IGNORE credit card 'EXPIRY' or 'EXP' dates (e.g., '09/28'). The true transaction date is almost always at the very bottom or top of the receipt alongside the time and till number.\n"
        ...
        "10. CRITICAL - TOTAL EXTRACTION vs PAYMENT METHODS: Extract the final net total cost. You MUST explicitly ignore credit card authorization codes, AID, TID, Cryptograms, or Auth Codes. Never extract these random strings as the final total. If a discount appears after the balance, extract the final charged 'CREDIT/DEBIT' payment amount as the true total.\n"
