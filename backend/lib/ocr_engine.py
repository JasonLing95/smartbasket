...
def extract_receipt_data_via_llm(text_blob: str) -> dict | None:
    ...
    system_instruction = (
        ...
        "10. CRITICAL - TOTAL EXTRACTION vs PAYMENT METHODS: Extract the final net total cost. You MUST explicitly ignore credit card authorization codes, AID, TID, Cryptograms, or Auth Codes. If a discount appears after the balance, you MUST extract the final charged 'CREDIT/DEBIT' payment amount (e.g., £0.54) as the true total. NEVER extract 'Balance Due', 'Subtotal', or 'Total' rows as physical grocery items inside the `items` array.\n"
