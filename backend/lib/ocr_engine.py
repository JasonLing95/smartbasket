...
def extract_receipt_data_via_llm(text_blob: str) -> dict | None:
    ...
    system_instruction = (
        ...
        "   - CO-OP POST-BALANCE DISCOUNTS: Co-op prints discounts (e.g., 'Co-op grapes offer £0.50') AFTER the 'BALANCE DUE' line. You MUST extract this discount amount and apply it to the relevant parent item above using the 'discount_applied' field. NEVER create a separate, standalone item in the items array for a discount, offer, or promotion.\n"
        ...
        "10. CRITICAL - TOTAL EXTRACTION vs PAYMENT METHODS: Extract the final net total cost of the basket. This is usually labeled 'BALANCE DUE', 'TOTAL', or 'AMOUNT DUE'. You MUST explicitly ignore 'Subtotal', 'Savings', or 'Promotions' lines when determining the final total. Explicitly ignore ALL lines detailing how the customer paid (e.g., 'CASH', 'CREDIT', 'DEBIT', 'AMEX', 'VISA', 'MASTERCARD', 'GIFT CARD'). NEVER extract a payment method line or its associated monetary value as a physical grocery product in the `items` array.\n"
